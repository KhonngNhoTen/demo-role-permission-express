...
class Permission {
  ...
  constructor(schema, wrapperPermission = null) {
    ...
    if (schema.url) {
      ...
      this.method = method;
      this.url = wrapperPermission ? wrapperPermission.baseUrl + url : url;
    }
    ...
    else this.type = this.method && this.url ? Permission.TYPE_PERMISSION.API : Permission.TYPE_PERMISSION.MENU;
    ...
    if (Array.isArray(schema)) {
      this.permissions = this.permissions.concat(this.createChilds(schema));
      this.enable = false;
    } else {
      this.permissions = this.permissions.concat(this.createChilds([...schema.ui, ...schema.apis]));
      this.enable = true;
    }
  }
  ...
  createChilds(permissions) {
    ...
    for (let i = 0; i < permissions.length; i++) {
      ...
      result.push(new Permission(permission));
    }
    return result;
  }
